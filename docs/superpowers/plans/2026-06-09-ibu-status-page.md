# Ibu Status Page (CekRisiko) Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Implement the CekRisiko.html design as `app/ibu/status/page.tsx` — a read-only pregnancy status page showing IMT profile, last posyandu measurements, and a risk-level status section with educational content and an interactive preview toggle.

**Architecture:** Two files: a thin page shell that loads data and a self-contained view component. The view receives `profile` and `latestVisit` as props and manages the `activeTab` state locally. Risk level is computed from `latestVisit` data using the same logic already in `pregnancy-dashboard-view.tsx`.

**Tech Stack:** Next.js App Router, React, TypeScript, Tailwind CSS, lucide-react, `@/lib/actions/pregnancy`, `@/lib/growth-standards/imt-calc`
...
|--------|------|----------------|
| Create | `app/ibu/status/page.tsx` | Data loading, routing guard, loading/error states |
| Create | `components/ibu/pregnancy-status-view.tsx` | Full UI: header, IMT card, measurement card, zone bar, education content |
...
**Files:**
- Create: `app/ibu/status/page.tsx`
...
Expected: No errors related to `app/ibu/status/page.tsx`. (The component import will error until Task 2 is done — that's fine, create a stub first if needed: `export default function PregnancyStatusView() { return null }` in `components/ibu/pregnancy-status-view.tsx`.)

- [ ] **Step 3: Commit**

```bash
git add app/ibu/status/page.tsx
git commit -m "feat: add ibu status page shell with data loading"
```

---

## Task 2: Component Skeleton, Helpers, and Header

**Files:**
- Create: `components/ibu/pregnancy-status-view.tsx`

- [ ] **Step 1: Create the component file with all types, helpers, and header**
...
export default function PregnancyStatusView({ profile, latestVisit }: Props) {
  const computed = latestVisit
    ? computeRiskLevel(latestVisit.lilaCm, latestVisit.hbGdl, latestVisit.isOnTrack)
    : 'rendah'
  const [activeTab, setActiveTab] = useState<RiskLevel>(computed)

  return (
    <div className="h-full overflow-hidden flex flex-col bg-[#E7ECF1]">

      {/* ── Header ── */}
      <header
        className="flex-none px-6 pt-1.5 pb-[18px] border-b-0 z-[5]"
        style={{
      ...
      {/* ── Scrollable body ── */}
      <main className="flex-1 overflow-y-auto overflow-x-hidden px-5 pt-[18px] pb-[110px]">
        {/* Cards will be added in Task 3 */}
        {/* Status section will be added in Tasks 4–5 */}
      </main>
...
```bash
git add components/ibu/pregnancy-status-view.tsx
git commit -m "feat: add pregnancy status view skeleton with header"
```

---

## Task 3: IMT Card and Last Measurement Card

**Files:**
- Modify: `components/ibu/pregnancy-status-view.tsx` — replace the `{/* Cards will be added in Task 3 */}` comment with the two card sections below.

- [ ] **Step 1: Add section label helper and both data cards inside `<main>`**

Replace `{/* Cards will be added in Task 3 */}` with:

```tsx
        {/* ── Section i: IMT Awal Kehamilan ── */}
        <div className="flex items-center gap-2 mx-0.5 mb-3">
          <span className="flex-none w-5 h-5 rounded-[7px] bg-[#1178D4] text-white text-[11px] font-bold flex items-center justify-center font-mono">i</span>
        ...
        <div className="flex items-center gap-2 mx-0.5 mb-3">
          <span className="flex-none w-5 h-5 rounded-[7px] bg-[#1178D4] text-white text-[11px] font-bold flex items-center justify-center font-mono">1</span>
          ...
          {latestVisit ? (
            <>
              ...
              <div className="flex gap-2.5">
                {/* BB */}
                <div className="flex-1 bg-white border border-[#E4EDE7] rounded-[12px] px-2 py-3 text-center">
                  ...
                  <div className="text-[9px] font-medium text-[#1178D4] mt-0.5 leading-tight">
                ...
                <div className="flex-1 bg-white border border-[#E4EDE7] rounded-[12px] px-2 py-3 text-center">
                  ...
                  <span className={`inline-block mt-1.5 text-[8.5px] font-bold px-2 py-0.5 rounded-full ${
...
```bash
git add components/ibu/pregnancy-status-view.tsx
git commit -m "feat: add IMT profile and last measurement cards to status view"
```

---

## Task 4: Zone Bar, Preview Toggle, and Verdict

**Files:**
- Modify: `components/ibu/pregnancy-status-view.tsx` — add status section after the measurement card, inside `<main>`.

- [ ] **Step 1: Add the status section after `</section>` (end of measurement card)**

Add this JSX after the measurement card's closing `</section>` tag and before the closing `</main>`:
...
```tsx
        ...
        {latestVisit && (
          <section className="mt-2">
            <div className="flex items-center gap-2 mx-0.5 mb-3">
              <span className="flex-none w-5 h-5 rounded-[7px] bg-[#1178D4] text-white text-[11px] font-bold flex items-center justify-center font-mono">2</span>
              <h2 className="text-[14px] font-semibold text-[#1F2937]">Status Kondisi Anda</h2>
            </div>

            {/* Preview toggle */}
            <div className="flex gap-1.5 bg-[#F4F7F5] border border-[#E4EDE7] p-1 rounded-[12px] mb-3">
              {(['rendah', 'sedang', 'tinggi'] as RiskLevel[]).map((key) => {
                const isOn = activeTab === key
                const color = key === 'rendah' ? '#1E9E62' : key === 'sedang' ? '#8A6100' : '#9F1C1C'
                return (
                  <button
                    key={key}
                    onClick={() => setActiveTab(key)}
                    className={`flex-1 flex items-center justify-center gap-1.5 text-[11px] font-semibold py-1.5 px-1 rounded-[9px] transition-all duration-200 ${
                      isOn
                        ? 'bg-white shadow-[0_2px_5px_rgba(9,30,66,0.1)]'
                        : 'text-[#989DA3]'
                    }`}
                  ...
                  >
                    <span
                      ...
                      style={{ background: isOn ? color : '#989DA3', opacity: isOn ? 1 : 0.55 }}
                    />
            ...
            <div className="bg-white border border-[#E4EDE7] rounded-[18px] p-4 shadow-[0_4px_14px_-8px_rgba(9,30,66,0.12)]">
              ...
              <div className="relative h-3 rounded-full overflow-visible" style={{
                background: 'linear-gradient(90deg, #1E9E62 0% 33%, #F2B705 33% 66%, #E0524E 66% 100%)',
              }}>
                <span
                  className="absolute top-1/2 w-5 h-5 rounded-full bg-white border-[3px] shadow-[0_3px_9px_rgba(9,30,66,0.3)] -translate-y-1/2 -translate-x-1/2"
                  style={{
                    left: activeTab === 'rendah' ? '16%' : activeTab === 'sedang' ? '50%' : '83%',
                    borderColor: activeTab === 'rendah' ? '#1E9E62' : activeTab === 'sedang' ? '#F2B705' : '#E0524E',
                    transition: 'left 0.55s cubic-bezier(0.22,0.61,0.36,1), border-color 0.3s',
                  }}
                />
              </div>
              ...
              <div className="flex justify-between mt-2.5">
                {([
                  { key: 'rendah', label: 'Aman',             color: '#1E9E62', dot: '#1E9E62' },
                  { key: 'sedang', label: 'Perlu Perhatian',  color: '#8A6100', dot: '#F2B705' },
                  { key: 'tinggi', label: 'Risiko Tinggi',    color: '#9F1C1C', dot: '#E0524E' },
                ] as const).map(({ key, label, color, dot }) => (
                  <span
                    key={key}
                    className="text-[10px] font-semibold flex items-center gap-1 transition-opacity duration-200"
                    style={{ color, opacity: activeTab === key ? 1 : 0.4 }}
                  >
              ...
              {/* Verdict */}
              <div className={`flex items-center gap-3 mt-4 pt-[15px] border-t border-[#E4EDE7]`}>
            ...
            {/* Education card placeholder — filled in Task 5 */}
          </section>
        )}
```
...
```bash
git add components/ibu/pregnancy-status-view.tsx
git commit -m "feat: add zone bar, preview toggle, and verdict to status view"
```

---

## Task 5: Education Card Content

**Files:**
- Modify: `components/ibu/pregnancy-status-view.tsx` — replace `{/* Education card placeholder — filled in Task 5 */}` with the education card below.

- [ ] **Step 1: Replace the placeholder comment with the education card**
                ...
                <>
                  ...
                  <ul className="flex flex-col gap-2.5 mt-2.5 list-none">
                    {[
                      ...
                      <li key={i} className="relative pl-4 text-[12.5px] text-[#4C545F] leading-[1.6]">
                        <span className="absolute left-0.5 top-[7px] w-1.5 h-1.5 rounded-full bg-[#1E9E62]" />
                        {t}
                      </li>
                    ))}
                  </ul>
                ...
                <>
                  ...
                  <ul className="flex flex-col gap-2.5 mt-2.5 list-none">
                    {[
                      ...
                      <li key={i} className="relative pl-4 text-[12.5px] text-[#4C545F] leading-[1.6]">
                        <span className="absolute left-0.5 top-[7px] w-1.5 h-1.5 rounded-full bg-[#D99100]" />
                ...
                <>
                  ...
                  <ul className="flex flex-col gap-2.5 mt-2.5 list-none">
                    {[
                      ...
                      <li key={i} className="relative pl-4 text-[12.5px] text-[#4C545F] leading-[1.6]">
                        <span className="absolute left-0.5 top-[7px] w-1.5 h-1.5 rounded-full bg-[#DC2626]" />
                  ...
                  <div className="mt-4 rounded-[14px] border-[1.5px] border-[#F6D2D2] bg-[#FEF1F1] p-3.5">
                    ...
                    <ul className="grid grid-cols-2 gap-x-3 gap-y-1.5 mt-2.5 list-none">
                      {[
                        ...
                        <li key={i} className="relative pl-3.5 text-[11px] font-medium text-[#9F1C1C] leading-snug">
                          <span
                            className="absolute left-0.5 top-[5px] w-1.5 h-1.5 rounded-[2px] bg-[#DC2626]"
...
```bash
git add components/ibu/pregnancy-status-view.tsx
git commit -m "feat: add education cards with risk-level content to status view"
```
...
- [x] **Spec coverage:** Header ✓, IMT card ✓, measurement card ✓, zone bar ✓, preview toggle ✓, verdict ✓, education (rendah/sedang/tinggi) ✓, danger box ✓, callout ✓, action buttons ✓, null states for profile/visits ✓, non-pregnant guard ✓
- [x] **No placeholders:** All text content is present verbatim from design, all code is complete
- [x] **Type consistency:** `RiskLevel`, `PregnancyProfileData`, `PregnancyVisitData`, `computeRiskLevel`, `IMT_BADGE` — consistent across all tasks. `isOnTrack` is `boolean` (not nullable) per `PregnancyVisitData` type definition.
- [x] **activeTab:** Initialized from `computed` which is only derived when `latestVisit` exists; status section only rendered when `latestVisit` exists — no stale default issue.
