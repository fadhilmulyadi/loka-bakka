...
const TEMPLATES: Record<Kind, { headers: string[]; example: string[] }> = {
  ...
  ibu: {
    headers: ["nama", "username", "password", "no_telp", "tanggal_lahir", "alamat", "sedang_hamil", "hpht", "tinggi_badan_cm", "bb_sebelum_hamil_kg", "bb_saat_ini_kg"],
    example: ["Contoh Ibu", "contoh.ibu", "rahasia123", "081234567890", "1995-05-10", "RT 01/RW 02, Desa Contoh", "ya", "2026-01-01", "158", "55", "60"],
  },
}
...
export function ImportPasienDialog({ kind, onImported }: { kind: Kind; onImported: () => void }) {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => { setOpen(v); if (!v) reset() }}>
      <DialogTrigger className="gap-1.5 text-xs h-8 px-4 inline-flex items-center bg-white rounded-[50px] shadow-[2px_2px_8px_rgba(0,0,0,0.08)] text-[#173753] hover:bg-white/80 cursor-pointer">
